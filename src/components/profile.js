...
const Profile = () => {
    ...
    useEffect(() => {
        ...
        axios.get('https://oussamaapi.azizbenmallouk.com/api/profile')
            .then(response => {            
        ...
        return (
            <section id="profile" className="py-20 bg-black text-white">
                <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
                    <p className="text-center text-lg text-gray-400">Loading profile information...</p>
                </div>
            </section>
        );
    }
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                    <div className="lg:col-span-4 text-center lg:text-left">
                        ...
                        <div className="flex justify-center lg:justify-start space-x-5 mt-6 mb-8">
                            ...
                            <a href={`mailto:${profile.email}`} className="text-gray-400 hover:text-cyan-400 transition-colors duration-300">
                    ...
                    <div className="lg:col-span-8">
                        ...
                        <div>
                            <h2 className="text-2xl font-bold text-white mb-4 flex items-center">
                                ...
                                <div className="bg-gray-900 p-5 rounded-lg shadow-sm border border-purple-500/20">
                                    <h3 className="font-semibold text-white mb-3">Tools & Frameworks</h3>
                                    <div className="flex flex-wrap gap-2">
                                        {Array.isArray(userTools) && userTools.length > 0 ? (
                                            userTools.map((tool, index) => (
                                                <span key={index} className="bg-gray-800 text-cyan-400 px-3 py-1 rounded-full text-sm flex items-center border border-purple-500/30">
                                                    {tool.name}
                                                    {tool.image_url && (
                                                        <img 
                                                            src={tool.image_url} 
                                            ...
                                            <span className="text-gray-400">No tools specified yet</span>
                                        )}
                                    </div>
                                </div>
                        </div>
